...
class Main:
    ...
    def _getToken() -> str:
        ...
        account_response: Dict = get("https://api.gofile.io/getAccountDetails?token=" + api_token,
                ...
                with open(filename, 'ab') as handler:
                    ...
                    for i, chunk in enumerate(response_handler.iter_content(chunk_size=chunk_size)):
                        ...
                        if rate < (1024):
                            ...
                            unit = "GB/s"
                        _print(
                            f"\rDownloading {file_info['filename']}: {part_size + i * len(chunk)} of {has_size} {round(progress, 1)}% {round(rate, 1)}{unit}")
