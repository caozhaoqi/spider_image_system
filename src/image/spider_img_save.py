...
@logger.catch
def download_images_from_file(file_path, cdds_index, final_download_url):
    """
        ...
        if index >= cur_download_finish_images_index:
            ...
            if constants.stop_download_image_flag:
                data = ImageModel(cur_download_images_index, file_path, url, image_url_re(url),
                                  time_to_utc(time.time()), cdds_index)
                record_end_download_image(constants.data_path + "\\download_final_image.json", data)
                logger.warning(f"set stop image stop_download_image_flag True! save result: {data.image_url}, txt name: {file_path}.")
                break
            if url:  # 跳过空行
...
def download_img_txt(self):
    ...
    if download_final_flag:
        download_final_flag_model = ImageModel(download_final_flag['index'], download_final_flag['txt_name'],
        ...
        # final_download_image_name = download_final_flag_model.image_name
        final_download_url = download_final_flag_model.image_url
        # final_download_image_index = download_final_flag_model.index
    ...
    for cdds_path in cdds:
        ...
        try:
            ...
            if final_download_txt_name:
                ...
                # continue
            download_images_from_file(new_file_name, cdds_index, final_download_url)
        except Exception as e:
